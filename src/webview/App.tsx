 ...
 * Compatible with both Bun and Vite environments
 */
const isDev = (): boolean => {
  // @ts-ignore - handle both Bun and Vite environments
  return (
...
const App: React.FC = () => {
  ...
  const [selectedRecording, setSelectedRecording] = useState<RecordingI | null>(
    null,
  );
  ...
  useEffect(() => {
    console.log(
      "[APP] Setting frontend token for API calls:",
      frontendToken ? "token available" : "no token",
    );
  ...
  useEffect(() => {
    ...
    const handleVoiceCommand = (data: {
      ...
      console.log(
        `[APP] [DEBUG] Received voice command: ${data.command}, timestamp: ${data.timestamp}`,
      );
      ...
      if (data.command === "start-recording") {
        ...
        if (currentScreen !== "recording") {
          console.log(
            "[APP] [DEBUG] Navigating to recording screen for voice-started recording",
          );
          navigateToRecording();
        } else {
          console.log(
            "[APP] [DEBUG] Already on recording screen, not navigating",
          );
        }
      } else if (
        ...
        console.log(
          "[APP] [DEBUG] ⚠️ Received stop-recording command - this is just UI notification, backend already stopped recording",
        );
      }
    };
    ...
    const handleRecordingStopped = (data: {
      ...
      console.log(
        `[APP] [DEBUG] Received recording-stopped-by-voice event for recording: ${data.id}`,
      );
    };
    ...
    const voiceCommandListener = (event: MessageEvent) => {
      try {
        ...
        console.log(
          `[APP] [DEBUG] Processing voice-command event: ${JSON.stringify(data)}`,
        );
        handleVoiceCommand(data);
      } catch (error) {
        console.error(
          "[APP] [DEBUG] ⚠️ Error handling voice command event:",
          error,
        );
      }
    };
    ...
    const recordingStoppedListener = (event: MessageEvent) => {
      try {
        const data = JSON.parse(event.data);
        console.log(
          `[APP] [DEBUG] Processing recording-stopped-by-voice event: ${JSON.stringify(data)}`,
        );
        handleRecordingStopped(data);
      } catch (error) {
        console.error(
          "[APP] [DEBUG] ⚠️ Error handling recording stopped event:",
          error,
        );
      }
    };
