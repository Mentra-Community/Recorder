...
export function useRecordings(options: UseRecordingsOptions = {}) {
  ...
  const [sessionConnected, setSessionConnected] = useState<boolean | null>(
    null,
  );
  ...
  const checkSessionStatus = useCallback(async () => {
    try {
      ...
      // Use api.session.isConnected which includes auth headers
      const isConnected = await api.session.isConnected();
      setSessionConnected(isConnected);
      return isConnected;
    } catch (err) {
  ...
  const fetchRecordings = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to fetch recordings"),
      );
    } finally {
      setLoading(false);
    }
  ...
  useRealTimeEvents("recording-status", (updatedRecording: any) => {
    setRecordings((prevRecordings) => {
      ...
      const index = prevRecordings.findIndex(
  ...
  useRealTimeEvents<{ id: string }>("recording-deleted", (data) => {
    setRecordings((prevRecordings) =>
      prevRecordings.filter((r) => r.id !== data.id),
    );
  });
  ...
  const startRecording = useCallback(async (): Promise<string> => {
    ...
    try {
      ...
      if (
        ...
        err.message.includes("No active AugmentOS SDK session")
      ) {
        setSessionConnected(false);
      }
  ...
  const deleteRecording = useCallback(async (id: string): Promise<void> => {
    try {
      ...
      setRecordings((prevRecordings) =>
        prevRecordings.filter((r) => r.id !== id),
      );
    } catch (err) {
      setError(
  ...
  const renameRecording = useCallback(
    async (id: string, title: string): Promise<void> => {
      try {
        ...
        setError(
          err instanceof Error ? err : new Error("Failed to rename recording"),
        );
