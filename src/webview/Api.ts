 ...
 * Bun uses import.meta.env.NODE_ENV, Vite uses import.meta.env.DEV
 */
const isDev = (): boolean => {
...
export const getBackendUrl = (): string => {
  // @ts-ignore - handle both Bun (BUN_PUBLIC_*) and Vite (VITE_*) environments
  const backendUrl =
    import.meta.env?.BUN_PUBLIC_BACKEND_URL ||
...
const API_BASE_URL = isDev() ? "" : getBackendUrl();
...
const api = {
  ...
  recordings: {
    ...
    getDownloadUrl: async (id: string): Promise<string> => {
      try {
        ...
        const response = await axiosInstance.get(
          `/api/recordings/${id}/download-token?t=${cacheBuster}`,
          {
            headers: getAuthHeader(),
          },
        );
        ...
        console.log(
          `[API] Token length: ${response.data.token ? response.data.token.length : 0}`,
        );
        ...
        console.log(
          `[API] Created download URL (length: ${downloadUrl.length})`,
        );
        return downloadUrl;
      } catch (error) {
  ...
  events: {
    connect: (): EventSource | null => {
      ...
      const baseUrl = isDev() ? "" : getBackendUrl();
