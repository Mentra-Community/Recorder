...
const RecordingImproved: React.FC<RecordingImprovedProps> = ({
  ...
  const [recordingState, setRecordingState] =
    useState<RecordingState>("loading");

  // Debug state changes
  useEffect(() => {
    console.debug(
      `[UI] Recording state changed to: ${recordingState}, recordingId: ${recordingId}`,
    );
  ...
  useRealTimeEvents(
    ...
    (data: { id: string; timestamp: number }) => {
      console.log(
        `[UI] [DEBUG] Recording started by voice command: ${data.id}`,
      );

      if (recordingState !== "recording") {
        console.log(
          `[UI] [DEBUG] Using voice-started recording ID: ${data.id}`,
        );
        ...
        console.log(
          `[UI] [DEBUG] ⚠️ Voice command started a different recording (${data.id}) than what we have (${recordingId})`,
        );
      }
    },
  );
  ...
  const fetchInitialTranscript = useCallback(async (id: string) => {
    try {
      console.log(
        `[UI] [DEBUG] Getting initial transcript for recording ${id}`,
      );
  ...
  useEffect(() => {
    const checkActiveRecording = async () => {
      try {
        console.log("[UI] Checking for active recording on mount");
        const recordings = await api.recordings.getAll();
        const activeRecording = recordings.find((r) => r.isRecording);

        if (activeRecording) {
          console.log(`[UI] Found active recording: ${activeRecording.id}`);
          ...
          console.log("[UI] No active recording found, will start new one");
          setRecordingState("ready-to-record");
        }
      } catch (error) {
        console.error("[UI] Error checking for active recording:", error);
        setRecordingState("ready-to-record");
      }
    };
  ...
  useEffect(() => {
    ...
    console.log("[UI] Ready to record - will auto-start recording");
    ...
    const startDelay = setTimeout(() => {
      ...
      const autoStartRecording = async () => {
        if (!onStartRecording) {
          console.debug("[UI] No onStartRecording function provided");
          return;
        }
        ...
        if (startingRecording.current) {
          console.warn(
            "[UI] Already starting recording, skipping duplicate attempt",
          );
          return;
        }
        ...
        if (recordingId) {
          console.debug("[UI] Already have recording ID, not starting new one");
          return;
        }

        console.log("[UI] Setting startingRecording flag to true");
        startingRecording.current = true;
        setRecordingState("starting");

        try {
          console.log("[UI] About to call onStartRecording to auto-start");
          const id = await onStartRecording();

          if (!isMounted) {
            console.debug("[UI] Component unmounted during start");
            return;
          }

          console.log(`[UI] Auto-started recording with ID: ${id}`);
          setRecordingId(id);
          setRecordingStartTime(Date.now());
          setTranscriptText("");
          setRecordingState("recording");
        } catch (err) {
          console.error("[UI] Error auto-starting recording:", err);

          if (!isMounted) {
            console.debug("[UI] Component unmounted during error handling");
            return;
          }
          ...
          if (axios.isAxiosError(err) && err.response?.status === 409) {
            console.warn(
              "[UI] Got 409 error - checking for existing recording",
            );
            try {
              const recordings = await api.recordings.getAll();
              const activeRecording = recordings.find((r) => r.isRecording);
              if (activeRecording) {
                console.log(
                  `[UI] Found existing active recording: ${activeRecording.id}`,
                );
                setRecordingId(activeRecording.id);
                setRecordingStartTime(activeRecording.createdAt);
                setTranscriptText(activeRecording.transcript || "");
              ...
              console.error(
                "[UI] Error fetching recordings after 409:",
                fetchError,
              );
            }
          }
          ...
          console.log("[UI] Setting startingRecording flag to false");
  ...
  const handleTranscriptUpdate = useCallback(
    (update: TranscriptUpdate) => {
      if (update.recordingId === recordingId) {
        console.log(
          `[UI] [DEBUG] Received transcript update for ${recordingId}: "${update.text}" (${update.isInterim ? "interim" : "final"})`,
        );
  ...
  const handleStartRecording = useCallback(async () => {
    if (recordingState !== "ready-to-record" || !onStartRecording) {
      console.log("[UI] [DEBUG] Cannot start recording:", {
        recordingState,
        hasStartFunction: !!onStartRecording,
      });
      return;
    }
    ...
    if (startingRecording.current) {
      console.log(
        "[UI] [DEBUG] Already starting recording via button, skipping duplicate",
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `[UI] [DEBUG] Successfully started recording, state should be 'recording'`,
      );
    } catch (err) {
      ...
      if (
        ...
        console.log(
          "[UI] User already has an active recording, showing error to user",
        );
        setError(
          "You have an active recording from a previous session. Please go back and stop it first.",
        );
      } else {
  ...
  useRealTimeEvents(
    "recording-status",
    (update: { id: string; status: RecordingStatusE }) => {
      console.log(
        `[RECORDING] [DEBUG] Received recording status update for ${update.id}: ${update.status}`,
      );
      console.log(
        `[RECORDING] [DEBUG] Current recordingId: ${recordingId}, current state: ${recordingState}`,
      );

      if (update.id === recordingId) {
        console.log(
          `[RECORDING] [DEBUG] Status update matches current recording`,
        );

        if (
          update.status === RecordingStatusE.RECORDING &&
          recordingState !== "recording"
        ) {
          console.log(
            `[RECORDING] [DEBUG] Backend confirms recording is active, updating to recording state`,
          );
          setRecordingState("recording");
          ...
          console.log(
            `[RECORDING] [DEBUG] Recording completed via SSE notification`,
          );
          setRecordingState("ready-to-record");
  ...
  useRealTimeEvents(
    "voice-command",
    (data: { command: string; timestamp: number }) => {
      console.log(
        `[RECORDING] [DEBUG] Received voice command: ${data.command}`,
      );

      if (data.command === "stop-recording" && recordingState === "recording") {
        console.log(
          `[RECORDING] [DEBUG] Voice command to stop recording received`,
        );
        console.log(
          `[RECORDING] [DEBUG] ⚠️ Not calling API to stop recording - backend is handling it`,
        );
  ...
  useRealTimeEvents(
    ...
    (data: { id: string; timestamp: number }) => {
      console.log(
        `[RECORDING] [DEBUG] Recording stopped by voice command: ${data.id}`,
      );

      if (data.id === recordingId) {
        console.log(
          `[RECORDING] [DEBUG] ⚠️ This matches our current recording - navigating back`,
        );
        setRecordingState("ready-to-record");
        ...
        if (onBack) {
          setTimeout(() => {
            console.log(
              `[RECORDING] [DEBUG] Navigating back to list after voice stop`,
            );
            onBack();
          }, 500);
        }
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              Tap the button below to start recording audio
            </p>
          </div>
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 flex flex-col">
      ...
      <div className="px-4 py-8 border-b border-gray-300 flex flex-col items-center justify-center bg-gray-100">
        <div className="flex items-center mb-2">
          <Mic
            size={24}
            className={`mr-3 ${recordingState === "recording" ? "text-red-500" : "text-gray-400"}`}
          />
      ...
      <div className="px-4 py-4 flex-1 overflow-y-auto bg-gray-50">
        <div className="flex items-center mb-2">
          <h3 className="text-sm font-medium text-gray-600 uppercase mr-3">
            Live Transcript
          </h3>
